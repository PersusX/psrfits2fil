...
def translate_header(psrfits_file):
    ...
    if subint_hdr['NPOL'] > 1:
        fil_header["nifs"] = 1
    fil_header["nsamples"] = subint_hdr['NAXIS2']*subint_hdr['NCHAN']*subint_hdr['NSBLK']
...
def main(fits_fn, outfn, nbits, \
    ...
    oldpcnt = ""
    #new_data = []
    #print(int(psrfits_file.nsubints))
    for isub in range(int(psrfits_file.nsubints)):
        ...
        #print(subint)
        subint /= scale_fac
        #new_data.append(subint)
        #print(subint)
        #print(isub,subint.shape)
    ...
    #plt.imshow(np.array(new_data).reshape(128*1024,4096).T[:,15000:28000],aspect='auto', origin='lower', cmap='viridis', vmin=0, vmax=50)
    #plt.show()
